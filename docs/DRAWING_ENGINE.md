...
| Detail views | `details.py`, pipeline, compiler | RULES 1.4 at the final scale: after the QA loop settles the scale, features printing smaller than 1.5 mm (and tightly toleranced features below 1:2) get a detail: the smallest drawing scale that prints the feature at ≥ 3 mm (max 10:1), a region ~44 mm across on the sheet centred on the feature's outermost edge point in the parent view (the view carrying its dimension, never a section), nearby features sharing it, at most 2. Placed in the clearest free space; the region is circled and lettered in the parent (ISO 128-44), the detail labelled `B (3:1)` with a thin boundary; its geometry is OCCT HLR about the detail centre clipped to the circle. Kept only if QA is not worse and the scale is unchanged. Dimensions stay in the parent view |
| Letters | | section / detail letters never reuse a datum letter (I, O, Q excluded) |
| Threads, keyways, grooves (R4) | `roles.py`, `threads.py`, geometry `features/grooves.py` | STEP carries no threads: a hole at the ISO 2306 tap drill of an ISO 261 coarse thread, deep enough for 0.8 × nominal engagement (and not an ISO 273 through clearance hole), is an assumed TAPPED_HOLE; its callout is derived (`M8x1.25-6H`, class 6H, ISO 965-1) with - owner decision 2026-09-26 - a blind thread depth of drill depth − 3 × pitch, `source=DEFAULT`, drill line added (`DRILL Ø6.80 DEPTH 20.00`). A pocket cut into a turned diameter along its axis is an assumed KEYWAY (EX 3: N9 width, depth +0.1/0, ⌖ 0.1 Ⓜ \|A\|). Face grooves are recognized in the geometry (coaxial convex + concave walls, annular floor → `GROOVE`), dimensioned (outer / inner Ø, depth), always sectioned, and are an assumed SEAL_GROOVE (EX 6: depth +0.05/0, ⌓ 0.05 \|A\|B\|, Ra 1.6); the face they open into becomes the SEALING_FACE (datum A, flatness 0.03, Ra 0.8, NO RADIAL LAY). Not yet: radial grooves on shafts / in bores, rounded-end keyways, thread recognition from modelled threads or PMI |
| Auxiliary views (R5) | `sections.py`, compiler | RULES 1.5: features whose axis is not principal (a hole square to a bevel) get an auxiliary view looking along that axis, drawn in free space and identified by the arrow method (ISO 128-30: lettered arrow on the view that sees the axis true length, same letter on the view). The dimensions no principal view shows true size (e.g. the hole callout) go there; an angled hole's entry point is located along the two principal axes most square to it (the third follows from the face). Full (not partial) auxiliary views |
| Horizontal shafts (R5) | `baseline.py`, `frames.py` | EX 3: a SHAFT is drawn with its axis horizontal in the front view - the planner picks a view frame (the user's if it already does, else Z-up, Y-up, X-up or Z-up/Y-right) |
| Conventional breaks (R5) | `sections.py`, compiler, `sheet.py` | RULES 1.6 for turned parts: when the long-part trigger fires, the middle of the longest constant diameter (keeping max(Ø, 15 %) of it at each end, clear of every other feature, at least 20 % of the part) is left out of every view showing the axis horizontally; the far part moves back so the cut ends are min(20 % of the removal, 0.6 Ø) apart, with zig-zag break lines (ISO 128-20). Dimensions across the break keep their true values (QA measures them back). Not in sectioned views, not for prismatic extrusions yet |
| Sheet metal (R6) | geometry `features/sheetmetal.py`, `flat_pattern.py`, compiler | Bends = coaxial partial cylinders r / r + t of equal extent (recognized before fillets); the part is sheet metal when every bend has the same t and ≥ 80 % of the planar area pairs up t apart (`GeometryIR.sheet_metal`). Bend callout `BEND R2.00 90°` where the bend is seen end-on. EX 5 flat pattern: flat segments + bend allowances with the DIN 6935 neutral fibre (k = 0.65 + 0.5 lg(r/s), r + k·s/2), bend direction UP/DOWN, holes mapped onto the blank; drawn as its own view "FLAT PATTERN" (developed length, width, bend-line positions; bend lines `UP 90° R2.00`) with the notes "DEVELOPED PER DIN 6935 (k = …) - confirm with the fabricator" and "BEND ANGLES ±0.5°". Supported: chains of bends with parallel axes (L, U, Z, hat); not yet: branched parts, non-parallel bends, reliefs / cut-outs across bends, slots on the flat |
| CAD change automation (R7, EX 2) | `revisions.py`, API `services/revisions.py` | A model uploaded with `previous_model_id` is the next version of a part. When its analysis completes, the previous version's latest drawing is regenerated for it in the same commit: feature diff (unchanged ids kept; *resized* = same place other size, *moved* = same size other place; added / removed; planar faces matched by plane), the drawing settings carried over (roles, datums, frames, finish, notes, threads, tolerances re-pointed to the new ids; items on removed features dropped and reported - a dropped datum makes the default datum selection run again), a revision entry appended (next letter without I / O / Q, the change summary, today's date; a previous title-block revision is listed as AS PREVIOUSLY ISSUED) and `change_report.json` stored. Regeneration is complete and deterministic from the carried settings - there are no manual sheet edits to preserve |
| Sections hide | `sections.py`, `baseline.py` | features the cutting plane misses are not visible in the section (removed, or behind the cut face with no hidden lines): their dimensions go to another view, and a view is added from the pool if only the section could carry them |
